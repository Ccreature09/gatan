...
app.prepare().then(() => {
  ...
  // Create Socket.IO server with environment-aware CORS configuration
  const io = new Server(httpServer, {
    cors: {
      origin: dev 
        ? ["http://localhost:3000", "http://127.0.0.1:3000"] // Development origins
        : process.env.ALLOWED_ORIGINS?.split(',') || ["*"], // Production origins
      methods: ["GET", "POST"],
  ...
  io.on('connection', (socket) => {
    ...
    // Add connection limit protection
    if (connectionCount > 50) {
      console.warn(`High connection count: ${connectionCount} - possible connection leak`);
    ...
    socket.on('game_action', (action) => {
      ...
      connectionCount--;
      console.log(`Player disconnected: ${socket.id} (Total: ${connectionCount})`);
  ...
  httpServer.listen(port, (err) => {
    ...
    console.log(`> Socket.IO server ready on same port`);
